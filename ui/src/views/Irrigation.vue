    ...
    <VContainer fluid class="pa-1">
      ...
      <VRow dense>
          ...
          <VCard title="Events" variant="tonal">
            <VCardText>
              <VExpansionPanels v-model="selectedEvent">
                <VExpansionPanel
                  v-for="(event, i) of config.events"
                  :key="i"
                  eager
                >
                  ...
                  <VExpansionPanelText>
                    ...
                    <VRow>
                      <VCol class="pa-2" :cols="cardWidth < 550 ? 12 : 9">
                        <VSelect
                          v-model="event.sequenceID"
                          :items="config.sequences"
                          label="Sequence"
                          hide-details="auto"
                          ...
                          :rules="[
                            (v) => !!v || 'Required',
                            (v) =>
                              !!config.sequences.find((s) => s.id === v) ||
                              'Not a valid sequence id',
                          ]"
                          validate-on="input"
                        />
                      </VCol>
                      <VCol class="pa-2" :cols="cardWidth < 550 ? 12 : 3">
                        <VTextField
                          ...
                          validate-on="input"
                          label="Priority"
                          :min="Irrigation.toEventPriority(-Infinity)"
                          :max="1000000"
                          @update:model-value="
                            (v) => Irrigation.toEventPriority(v)
                          "
                        />
                      </VCol>
                    </VRow>
                    <VRow>
                      <VCol class="pa-2">
                        <VTextField
                          :model-value="Math.floor(event.startOffset / 60)"
                          type="number"
                          hide-details="auto"
                          validate-on="input"
                          label="Start Offset (m)"
                          :min="Irrigation.Offset(Infinity)"
                          :max="Irrigation.Offset(-Infinity)"
                          @update:model-value="
                            (v) =>
                              (event.startOffset = Irrigation.Offset(
                                60 * Number(v),
                              ))
                          "
                        />
                    ...
                    <VRow>
                      <VCol align="center">
                        <VDatePicker
                          :model-value="new Date(event.start * 1000)"
                          title="Starting on:"
                          color="primary"
                      ...
                      <VCol align="center">
                        <VDatePicker
                          :model-value="new Date(event.end * 1000)"
                          title="Ending on:"
                          color="primary"
                          show-adjacent-months
                          hide-actions
                    ...
                    <VChipGroup
                      ...
                      @update:model-value="
                        (value: number[]) => {
                          ...
                          value
                            .filter((v) => v >= 0 && v < 7)
                            .forEach((v) => (event.days[v] = true));
                        }
                      "
                    >
                      <VChip
                        v-for="(day, dayIndex) of daysOfWeek"
                        :key="dayIndex"
                        color="primary"
                      >
                        {{ day }}
        ...
        <VCol :cols="cols">
          <VCard
            ref="sequences"
            v-resize="() => (cardWidth = sequences?.$el.offsetWidth)"
            title="Sequences"
            variant="tonal"
          >
            <VCardText>
              <VExpansionPanels v-model="selectedSequence">
                <VExpansionPanel
                  v-for="(sequence, i) of config.sequences"
                  :key="i"
                  eager
                >
                  <VExpansionPanelTitle>
                    <VSwitch
                      ...
                      color="primary"
                      hide-details="auto"
                      class="ma-n3 flex-grow-0"
                      inset
                      style="min-width: 48px"
                      :model-value="stateSequences.has(sequence.id)"
                      @click.stop
                      @update:model-value="
                        (v) => {
                      ...
                      <VCol>
                        <VRow>
                          ...
                          <VCol class="pa-1" :cols="cardWidth < 525 ? 12 : 3">
                            <VTextField
                              :model-value="Math.floor(job.duration / 60)"
                              density="compact"
                              label="Duration (m)"
                              type="number"
                              hide-details="auto"
                              ...
                              @update:model-value="
                                (v) =>
                                  (job.duration =
                                    60 * Math.max(1, Math.min(1440, Number(v))))
                              "
                            />
                          </VCol>
                        </VRow>
          ...
          <VCard title="Valves" variant="tonal">
            <VCardText>
              <VExpansionPanels v-model="selectedValve">
                <VExpansionPanel
                  v-for="(valve, i) of config.valves"
                  :key="i"
                  eager
                >
                  <VExpansionPanelTitle>
                    <VSwitch
                      ...
                      inset
                      style="min-width: 48px"
                      :model-value="stateValves.has(valve.id)"
                      @click.stop
                      @update:model-value="
                        (v) => {
                  ...
                  <VExpansionPanelText>
                    <VTextField
                      ...
                      :rules="[
                        (v: string) =>
                          (v != null && /\S/.test(v)) || 'Required',
                      ]"
                      validate-on="input"
                      clearable
                    />
                    <VSelect
                      v-model="valve.deviceID"
          ...
          <VCard title="Devices" variant="tonal">
            <VCardText>
              <VExpansionPanels v-model="selectedDevice">
                <VExpansionPanel
                  v-for="(device, i) of config.devices"
                  :key="i"
                  eager
                >
                  <VExpansionPanelTitle>
                    ...
                    <span v-if="stateDevices.has(device.mac)">
                      <VBtn
                        class="text-decoration-underline px-2 my-n2"
                        color="primary"
                        variant="text"
                        :href="`http://${stateDevices.get(device.mac)?.ip}`"
                        @click.stop
                  ...
                  <VExpansionPanelText>
                    ...
                    <VSelect
                      v-model="device.mac"
                      :items="
                        [...stateDevices.values()].filter(
                          (d) => !config.devices.find((d2) => d2.mac === d.mac),
                        )
                      "
                      label="Device"
                      :rules="[(v) => !!v || 'Required']"
            ...
            <VCardActions class="d-flex flex-wrap justify-center">
              <VChip
                v-for="[id, device] of [...stateDevices].filter(
                  ([_, d]) => !config.devices.find((d2) => d2.mac === d.mac),
                )"
                :key="id"
                :append-icon="mdiPlus"
                color="primary"
...
const cols = computed(() =>
  smAndDown.value ? 12 : md.value ? 6 : lg.value ? 4 : xlAndUp.value ? 3 : 12,
);
...
// State syncing
const stateSequences = ref<
  Map<Irrigation.SequenceID, Irrigation.SequenceExecution>
>(new Map());
const stateValves = ref<Map<Irrigation.ValveID, Irrigation.ValveExecution>>(
  new Map(),
);
const stateDevices = ref<Map<string, Irrigation.DeviceConnection>>(new Map());
const disabled = ref<boolean>(false);
const updateState = (state: Irrigation.State) => {
  stateSequences.value = new Map(
    Object.entries(state.sequences).map(([k, v]) => [
      Irrigation.SequenceID(k),
      v,
    ]),
  );
  stateValves.value = new Map(
    Object.entries(state.valves).map(([k, v]) => [Irrigation.ValveID(k), v]),
  );
...
async function startSequence(id: Irrigation.SequenceID) {
  ...
  if (res.ok) stateSequences.value.set(id, res.value!);
}
...
function createEvent() {
  ...
  while (config.value.events.find((seq) => seq.id === id)) id++;
...
function createSequence() {
  ...
  while (config.value.sequences.find((seq) => seq.id === id)) id++;
...
function createValve() {
  ...
  while (config.value.valves.find((seq) => seq.id === id)) id++;
...
function createDevice(mac: string = "") {
  ...
  while (config.value.devices.find((seq) => seq.id === id)) id++;
...
async function saveConfig(event: SubmitEventPromise) {
  ...
  if (res.valid) {
    errorSaving.value = !(await socket.emitWithAck(
      "setIrrigationConfig",
      config.value,
    ));
  } else {
...
.v-date-picker {
  background-color: color-mix(
    ...
    rgb(var(--v-theme-surface-variant), 0.1)
  );
}
</style>
